...
/etc/eks/bootstrap.sh '%s' %s \
	...
	if constraints.KubeletConfiguration.MaxPods != nil {
...
func (p *LaunchTemplateProvider) getKubeletExtraArgs(constraints *v1alpha1.Constraints, additionalLabels map[string]string) string {
	...
	if constraints.KubeletConfiguration.MaxPods != nil {
		kubeletArgs.WriteString(fmt.Sprintf(" --max-pods=%d", aws.Int32Value(constraints.KubeletConfiguration.MaxPods)))
	}

	if constraints.KubeletConfiguration.PodsPerCore != nil {
		kubeletArgs.WriteString(fmt.Sprintf(" --pods-per-core=%d", aws.Int32Value(constraints.KubeletConfiguration.PodsPerCore)))
	}
